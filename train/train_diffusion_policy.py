...
try:
	...
	from train.configs.diffusion_train_config import DiffusionTrainConfig, config_to_dict, parse_args
from train.utils.checkpoints import restore_checkpoint, save_checkpoint
from train.utils.infer import _build_model_from_metadata, _load_checkpoint_metadata
...
def _make_optimizer(args: DiffusionTrainConfig, params_tree) -> tuple[optax.GradientTransformation, Any, Any]:
	...
	tx = optax.masked(train_tx, trainable_mask)
...
def _make_train_step(
	...
	def train_step(params_state, opt_state, ema_params, rng_key, batch_features):
		...
		updates = jax.tree_util.tree_map(
			lambda update, mask: update if mask else jnp.zeros_like(update),
			updates,
			trainable_mask,
		)
...
def main() -> None:
	args = parse_args()
	if args.resume_path is None and args.pretrained_checkpoint is None:
		raise ValueError("Provide either --resume_path or --pretrained_checkpoint.")

	configure_jax_compilation_cache(args.jax_compilation_cache_dir)

	checkpoint_path = args.resume_path or args.pretrained_checkpoint
	...
	train_step = _make_train_step(graphdef, nonparam_state, tx, trainable_mask, float(args.grad_clip_norm), float(args.ema_decay))
	...
	for epoch in range(int(start_epoch), int(args.epochs) + 1):
		...
		for step_in_epoch in range(int(args.steps_per_epoch)):
			try:
				...
				iterator = iter_with_prefetch(iter(loader), 0)
				batch = next(iterator)
			# batch.features['inst_valid'] = jnp.zeros_like(batch.features['inst_valid'])  # Mask out all instruction losses until we have a better curriculum
