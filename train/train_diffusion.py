...
from train.utils import (
    ...
    make_dataset_config,
    replicate_to_mesh,
    run_data_parallel_warmup_if_needed,
    sim_state_signature,
...
def main():
    ...
    use_rl_source = bool(
        args.rl_policy_path or args.rl_failure_dir or args.rl_tfrecord
    )
    if use_rl_source:
        scenario_iter = _build_rl_scenario_generator(args, global_batch_size)
    else:
        ds_cfg = make_dataset_config(args, waymax_config, global_batch_size)
        scenario_iter = dataloader.simulator_state_generator(ds_cfg)
    scenarios = iter_with_prefetch(scenario_iter, args.prefetch_size)
