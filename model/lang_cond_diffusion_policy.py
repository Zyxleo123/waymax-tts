...
class DiffusionPolicy(nnx.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        if lang_dim is not None:
            self.input_proj_lang = MLP([lang_dim, hidden_dim, hidden_dim], rngs=rngs)
        cond_input_blocks = 5 + (1 if lang_dim is not None else 0)
        self.cond_projection = MLP([hidden_dim * cond_input_blocks, hidden_dim, cond_dim], rngs=rngs)
    ...
    def _condition_impl(self, features: PolicyFeatures) -> jnp.ndarray:
        ...
        parts = [ego_feature, goal_feature, other_feature, map_features, traffic_light_features]
        if getattr(self, "lang_dim", None) is not None and features.language is not None:
            # Expect features.language to be [batch, lang_dim] or [batch, seq, lang_dim]
            lang_in = features.language
            # If language is sequence (e.g., tokens), collapse/average along seq axis
            if lang_in.ndim == 3:
                lang_in = jnp.mean(lang_in, axis=1)
            lang_feat = self.input_proj_lang(lang_in, deterministic=True)
            parts.append(lang_feat)

        cond = jnp.concatenate(parts, axis=-1)
