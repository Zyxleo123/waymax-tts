...
def _build_question_bank(answers: dict[str, Any]) -> list[dict[str, Any]]:
	...
	if "traffic_light_state" in answers:
		...
		qa_list.append({
			...
			"question": "Is the traffic light in front of the ego vehicle red? Answer with yes or no.",
...
def _load_qas_for_scenario(qa_json_path: str, scenario_index: int) -> dict[str, Any]:
	...
	return {
		...
		"qas": _build_question_bank(answers),
	}
		...
		for cache_path in cache_paths:
			...
			try:
				if self.cfg.qa_dir is not None:
					qa_dir = Path(self.cfg.qa_dir)
					qa_stem = re.sub(r"_t\d+$", "", cache_file.stem)
					qa_json_path = qa_dir / f"{qa_stem}.json"
					if not qa_json_path.exists():
						raise FileNotFoundError(f"QA json file not found for cache shard: {qa_json_path}")
				...
				with np.load(cache_file, allow_pickle=False) as npz_data:
					...
					for start_index in range(0, total_examples, self.cfg.batch_size):
						...
						batch = QABatch(features=features, aux=aux)
