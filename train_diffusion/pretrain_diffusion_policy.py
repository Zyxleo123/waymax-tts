...
from train_diffusion.configs.diffusion_pretrain_config import DiffusionPretrainConfig, config_to_dict, parse_args
...
def _add_dummy_instruction_features(feats: dict[str, jax.Array], args: DiffusionPretrainConfig) -> dict[str, jax.Array]:
    ...
    augmented["subgoal_xy"] = jnp.zeros((batch_size, 2), dtype=jnp.float32)
    augmented["subgoal_valid"] = jnp.zeros((batch_size,), dtype=jnp.bool_)
    return augmented
