...
def _warn_unknown_goal_feature(key: str) -> bool:
    """Return whether ``key`` is a known goal feature, warning once if it is not."""
    if key not in features.GOAL_FEATURE_SIZES:
        print(f"Warning: Unknown goal feature '{key}' not found in GOAL_FEATURE_SIZES")
        return False
...
class VecFeaturesExtractor(extractor.AbstractFeaturesExtractor):
    ...
    def __init__(
        ...
        Args:
            ...
            goal_config: Configuration for goal features. Empty/absent leaves the goal
                out of the observation entirely (the original V-Max behavior).
        """
        ...
        self._goal_config = goal_config or {"features": []}
        ...
        # Goal
        self._goal_features_key = [
            key for key in self._goal_config.get("features", []) if _warn_unknown_goal_feature(key)
        ]
        self._goal_max_speed = self._goal_config.get("max_speed", 30.0)
        self._goal_max_time = self._goal_config.get("max_time", 8.0)
        self._goal_dt = self._goal_config.get("dt", 0.1)
    ...
    @property
    def goal_features_size(self) -> int:
        """Return the size of the goal feature block (0 when goal features are disabled)."""
        return sum(features.GOAL_FEATURE_SIZES[key] for key in self._goal_features_key)
    ...
    def extract_features(
        ...
        Returns:
            A tuple containing:
              ...
              - Goal features (empty when goal features are disabled).
        ...
        goal_features = self._build_goal_features(state)
    ...
    def unflatten_features(self, vectorized_obs: jax.Array) -> tuple[tuple[jax.Array, ...], tuple[jax.Array, ...]]:
        ...
        goal_size = self.goal_features_size
        goal_features = vectorized_obs[..., unflatten_size : unflatten_size + goal_size]
        goal_features = goal_features.reshape(*batch_dims, goal_size)
        unflatten_size += goal_size
    ...
    def _build_goal_features(self, state: datatypes.SimulatorState) -> features.GoalFeatures:
        """Build goal features, expressed in the ego frame, from the simulator state.

        The goal is the SDC's last valid logged position -- the same definition used by
        the ``reached_goal`` metric and the goal rewards, so the policy is told exactly
        ...
        Returns:
            An instance of GoalFeatures.
        """
        goal_features = features.GoalFeatures(field_names=self._goal_features_key)

        if not self._goal_features_key:
            return goal_features

        sdc_idx = operations.get_index(state.object_metadata.is_sdc)
        goal_xy = operations.get_sdc_goal_xy(state, sdc_idx)
        ego_xy, ego_yaw = operations.get_sdc_pose_xy_yaw(state, sdc_idx)

        # Goal in the ego frame: translate, then rotate by -yaw.
        delta = goal_xy - ego_xy
        cos_yaw, sin_yaw = jnp.cos(ego_yaw), jnp.sin(ego_yaw)
        goal_ego_xy = jnp.stack(
            [
        ...
        bearing = jnp.arctan2(goal_ego_xy[1], goal_ego_xy[0])
        time_remaining = operations.get_remaining_time(state, self._goal_dt)

        # The sufficient statistic for the task: the average speed needed from here on
        # to arrive before the horizon runs out.
        required_speed = distance / time_remaining
        ego_speed = state.current_sim_trajectory.speed[sdc_idx].squeeze()

        for key in self._goal_features_key:
            if key == "xy":
                value = extractor.normalize_path(goal_ego_xy, self._max_meters)
            elif key == "distance":
                value = jnp.clip(distance / self._max_meters, max=1.0)
            elif key == "heading":
                value = jnp.stack([jnp.cos(bearing), jnp.sin(bearing)])
            elif key == "time_remaining":
                value = jnp.clip(time_remaining / self._goal_max_time, max=1.0)
            elif key == "required_speed":
                value = jnp.clip(required_speed / self._goal_max_speed, max=1.0)
            elif key == "speed_ratio":
                # >1 means the SDC is running ahead of schedule, <1 means it is falling
                # behind and needs to accelerate to still make the goal in time.
                value = jnp.clip(ego_speed / jnp.maximum(required_speed, 0.1), max=2.0)

            setattr(goal_features, key, value)
