...
# Size (number of scalars) of each goal feature. Used to compute the observation
# size analytically, so it must stay in sync with VecFeaturesExtractor._build_goal_features.
GOAL_FEATURE_SIZES = {
    "xy": 2,  # Goal position in the ego frame, normalized
    "distance": 1,  # Euclidean distance to the goal, normalized
    "heading": 2,  # (cos, sin) of the bearing to the goal in the ego frame
    "time_remaining": 1,  # Episode time left, normalized
    "required_speed": 1,  # distance_to_goal / time_remaining, normalized
    "speed_ratio": 1,  # current_speed / required_speed, clipped
}


@chex.dataclass
class GoalFeatures:
    """Dataclass representing the SDC's goal, expressed in the ego frame.

    ``path_target`` tells the policy where the road goes next; it says nothing about
    where the trip ends or how long is left to get there. Since the goal-reaching
    criterion is "be within a radius of the expert's final logged position before the
    horizon runs out", a policy without these features is being scored on state it
    cannot see. ``required_speed`` in particular is the sufficient statistic for the
    task: it is the average speed the SDC must hold from here on to arrive in time.
    """

    field_names: Sequence[str]  # Field names
    xy: jax.Array = field(default_factory=lambda: jnp.array(()))
    distance: jax.Array = field(default_factory=lambda: jnp.array(()))
    heading: jax.Array = field(default_factory=lambda: jnp.array(()))
    time_remaining: jax.Array = field(default_factory=lambda: jnp.array(()))
    required_speed: jax.Array = field(default_factory=lambda: jnp.array(()))
    speed_ratio: jax.Array = field(default_factory=lambda: jnp.array(()))

    @property
    def data(self) -> jax.Array:
        """Return the goal features concatenated into a single flat vector."""
        if not self.field_names:
            return jnp.array(())

        return jnp.concatenate([jnp.atleast_1d(getattr(self, name)) for name in self.field_names])

    def plot(self, ax) -> None:
        """Plot the goal position in the ego frame."""
        if "xy" not in self.field_names:
            return

        ax.plot(self.xy[0], self.xy[1], "*", ms=12, color="gold", markeredgecolor="black")
