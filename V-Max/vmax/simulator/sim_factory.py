...
def make_data_generator(
    ...
    Args:
        ...
        num_paths: SDC route tensor's path count. None uses the ScenarioMax
            layout; raw WOMD needs ``constants.WOMD_NUM_SDC_PATHS``.
        num_points_per_path: Points per SDC route. None uses the ScenarioMax
            layout; raw WOMD needs ``constants.WOMD_NUM_POINTS_PER_SDC_PATH``.
    ...
    return dataloader.simulator_state_generator(
        _config.DatasetConfig(
            ...
            num_paths=num_paths if num_paths is not None else constants.NUM_SDC_PATHS,
            num_points_per_path=(
                num_points_per_path if num_points_per_path is not None else constants.NUM_POINTS_PER_SDC_PATH
            ),
        ),
    )
...
def make_env(
    ...
    # Register metrics. Waymax's registry is global and `register_metric` raises on
    # a duplicate name, so building a second env in one process (e.g. an evaluation
    # env alongside the training env, as the banker does) would fail. Skipping the
    # already-registered names is safe: the metric object comes from the same
    # `get_metrics(key)` either way.
    already_registered = set(waymax_metrics.get_metric_names())
...
def make_env_for_training(
    ...
    Args:
        ...
        goal_shaping_config: Potential-based goal shaping config. Applied to training
            only: the shaping is policy-invariant, so leaving it out of the evaluation
            env keeps the reported episode reward equal to the true task reward.
    ...
    waymax_env = wrappers.BraxWrapper(waymax_env, termination_keys, goal_shaping_config=goal_shaping_config)
