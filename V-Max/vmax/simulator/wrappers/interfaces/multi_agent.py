...
class MultiAgentBraxWrapper(Wrapper):
    ...
    def metrics(self, state: datatypes.SimulatorState) -> types.Metrics:
        ...
        # Goal-reaching signal, logged independently of the reward config (see BraxWrapper).
        metrics_dict["reached_goal"] = jax.lax.stop_gradient(
            vmax_operations.is_within_goal(state).astype(jnp.float32)
        )
