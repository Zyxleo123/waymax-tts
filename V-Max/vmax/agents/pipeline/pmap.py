...
def device_put_replicated(tree, devices):
    ...
    Drop-in replacement for ``jax.device_put_replicated`` (removed in JAX >= 0.7).
    Each leaf gains a leading axis of size ``len(devices)`` so the result can be
    consumed directly by ``jax.pmap`` (which shards axis 0 across devices).

    Args:
        tree: A pytree of arrays to replicate.
        devices: Sequence of devices to replicate across.

    Returns:
        The replicated pytree with a leading device axis.
