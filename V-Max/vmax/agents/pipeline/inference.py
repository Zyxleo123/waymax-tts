...
def expert_step(
    ...
    # The SDC's object index varies per scenario, so it must be selected
    # *per environment*. ``operations.get_index`` argmaxes over the whole
    # flattened ``is_sdc`` and returns a single scalar, which only happens to be
    # correct when every batched scenario stores its SDC at the same object slot
    # (true for ScenarioMax, false for raw WOMD). Using that one index for the
    # whole batch silently applies another object's inverse action to the SDC of
    # every other env -> the ego drives the wrong trajectory, drifts off the log
    # and racks up ghost collisions. Select per-env instead.
    # is_sdc: (..., num_agents) -> sdc_idx: (...)
    sdc_idx = jnp.argmax(env_transition.state.object_metadata.is_sdc, axis=-1)
    # (..., 1, 2)
    action_sdc = jnp.take_along_axis(actions, sdc_idx[..., None, None], axis=-2)
    # (..., 2)
...
def idm_step(
    ...
    Returns:
        Tuple containing the next environment transition and the generated RL transition.
    """
    observation = jax.tree_util.tree_map(lambda x: x[0], env_transition.observation)
