...
class WayformerEncoder(nn.Module):
    ...
    def __call__(self, obs: jax.Array) -> jax.Array:
        ...
        sdc_traj_features, other_traj_features, rg_features, tl_features, gps_path_features, goal_features = features
        ...
        goal_encoding, goal_mask = encoders.build_goal_token(
            goal_features,
            self.dk,
            self.embedding_layer_sizes,
            self.embedding_activation,
        ...
        if self.fusion_type == "early":
            ...
            encodings = [sdc_traj_encoding, other_traj_encoding, rg_encoding, tl_encoding, gps_path_encoding]
            token_masks = [
                sdc_traj_valid_mask,
            ...
            if goal_encoding is not None:
                encodings.append(goal_encoding)
                token_masks.append(goal_mask)

            concat_embeddings = jnp.concatenate(encodings, axis=1)
            concat_masks = jnp.concatenate(token_masks, axis=1)
            ...
            outputs = [output_sdc_traj, output_other_traj, output_rg, output_tl, output_gps_path]

            # Late fusion concatenates *latent* attention outputs, so the goal token has to
            # go through its own attention stream to come out latent-shaped like the rest.
            if goal_encoding is not None:
                outputs.append(self_attn(depth=self.attention_depth, name="goal_attention")(goal_encoding))
            ...
            # As in late fusion: attend over the goal token so it is latent-shaped.
            if goal_encoding is not None:
                outputs.append(self_attn(depth=self.attention_depth, name="goal_attention")(goal_encoding))
