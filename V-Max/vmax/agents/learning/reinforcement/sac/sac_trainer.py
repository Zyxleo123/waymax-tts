...
def train(
    ...
    eval_scenario_failures: waymax_datatypes.SimulatorState | None = None,
    kl_coef: float = 0.0,
    kl_reference_checkpoint: str | None = None,
    banker: typing.Any = None,
) -> None:
    """Train a SAC agent.
    ...
    Args:
        ...
        auto_alpha: Whether to tune alpha against ``target_entropy`` instead of holding
            it fixed.
        target_entropy: Target entropy for alpha tuning. Defaults to ``-action_size``.
        ...
        init_checkpoint: Optional path to a prior run's ``model_*.pkl`` (as saved by
            ``train_utils.save_params``) to warm-start policy/value params from.
            Network config must match the checkpoint's shapes.
        eval_scenario_failures: Optional second eval set, run alongside
            ``eval_scenario`` and logged under ``eval_failures/*``.
    ...
    # Keeps a single model_best.pkl, overwritten whenever the held-out eval improves on
    # min(reached_goal, accuracy). Independent of the periodic model_<step>.pkl saves.
    best_saver = train_utils.BestCheckpointSaver(checkpoint_logdir) if checkpoint_logdir else None
    ...
    # Frozen anchor for the KL penalty. Defaults to whatever we warm-started from,
    # which is the usual intent: "fix this scenario without ceasing to drive like
    # the pretrained policy".
    reference_policy_params = None
    if kl_coef > 0.0:
        from vmax.scripts.evaluate import utils as eval_utils

        reference_path = kl_reference_checkpoint or init_checkpoint
        if reference_path is None:
            raise ValueError(
                "kl_coef > 0 needs a reference policy: pass kl_reference_checkpoint, "
                "or init_checkpoint to anchor to the warm start."
            )
        print(f"-> KL anchor: pi_ref = {reference_path} (kl_coef={kl_coef})")
        reference_policy_params = jax.device_put(eval_utils.load_params(reference_path).policy)
    ...
    for iter in tqdm(range(total_iters), desc="Training", total=total_iters, dynamic_ncols=True, disable=disable_tqdm):
        ...
        if do_evaluation and not iter % eval_freq:
            ...
            eval_metrics = _metrics.collect(
                eval_metrics,
                "ep_len_mean",
                termination_keys=_metrics.get_termination_keys(env),
            )

            if best_saver is not None:
                eval_metrics.update(
                    best_saver.update(current_step, eval_metrics, pmap.unpmap(training_state.params)),
                )

            progress_fn(current_step, eval_metrics)

            if eval_scenario_failures is not None:
                eval_metrics_failures = run_evaluation(eval_scenario_failures, training_state)
                jax.tree_util.tree_map(lambda x: x.block_until_ready(), eval_metrics_failures)
                eval_metrics_failures = pmap.flatten_tree(eval_metrics_failures)
                eval_metrics_failures = _metrics.collect(
                    eval_metrics_failures,
                    "ep_len_mean",
                    termination_keys=_metrics.get_termination_keys(env),
                )
                eval_metrics_failures = {
                    f"eval_failures/{name}": value for name, value in eval_metrics_failures.items()
                }
                progress_fn(current_step, eval_metrics_failures)

            if banker is not None:
                # Deterministic per-scenario pass: bank any scenario that came out
                # clean, once, forever. Separate from the eval above on purpose --
                # that one shuffles and resamples, so it has no stable identity.
                bank_metrics = banker.step(pmap.unpmap(training_state.params), current_step)
                progress_fn(current_step, bank_metrics)
