"""Run a trained PPO policy over a set of Waymax scenarios, report metrics, and
...
Example
-------
    python -m rl.eval_ppo --model runs/ppo_failures_delta/lr_3e-4/ppo_waymax.zip \
        --failure-dir /path/to/failure_samples --action-space delta
"""
...
def main():
    ...
    env = WaymaxGymEnv(
        source,
        reward_config=RewardConfig(goal_threshold_m=args.goal_threshold_m),
...
def _parse_args():
    p = argparse.ArgumentParser(description="Evaluate a PPO policy on Waymax scenarios.")
