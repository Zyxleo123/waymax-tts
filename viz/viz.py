...
def plot_trajectory(
  ...
  if indices is not None and time_idx is not None:
    # Simple decluttering in world coordinates to reduce overlapping labels.
    min_label_separation_m = 2.5
    ...
    for i in range(num_obj):
      if not traj.valid[i, time_idx]:
        continue
      base_xy = traj_5dof[i, time_idx, :2].astype(np.float32)
      chosen_xy = None
      for offset in offset_candidates:
        candidate = base_xy + offset
        if all(np.linalg.norm(candidate - p) >= min_label_separation_m for p in placed_positions):
          chosen_xy = candidate
          break
      if chosen_xy is None:
        continue
      placed_positions.append(chosen_xy)
      ...
      ax.text(
          ...
          fontsize=9,
          color="black",
          bbox=dict(facecolor="white", edgecolor="black", boxstyle="round,pad=0.10", linewidth=0.8, alpha=0.95),
      )
