...
% Compile: pdflatex pseudo_gt_progress_beamer.tex  (twice if needed)
...
\begin{frame}{V-Max SAC setup (\texttt{repro\_sac\_v2})}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Data: \texttt{ScenarioMaxWaymo/training.tfrecord}
      \item \textbf{ScenarioMax:} WOMD $\rightarrow$ unified format $\rightarrow$ V-Max tfrecord\\
            \small(same \texttt{scenario/id}; adds expert SDC paths; 64 objs, 10$\times$300 paths)
      \item Eval: \texttt{ScenarioMaxWaymoValid} ($\sim$256 scenes)
      \item Dynamics: invertible bicycle model
...
\begin{frame}{Policy architecture: LQ encoder + MLP head}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Scene tokens:} concatenate embedded tokens ($\sim$221) with validity masks
...
\begin{frame}{Pseudo-GT pipeline}
  ...
  Failure JSON $\rightarrow$ \texttt{tfrecord + scene_idx}
