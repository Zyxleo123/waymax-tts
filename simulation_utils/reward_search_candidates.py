...
def run_with_improvement_from_candidates(
    args, baseline_code, candidate_scenarios, use_baseline=False
):
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        for file_name, scenario_indices in candidate_scenarios.items():
            scenario_indices = sorted(scenario_indices)
            print(f"Running reward search for {file_name} with {scenario_indices}")
            file_path = os.path.join(args.tfrecord_path, "validation", file_name)
            if args.rs_iterations == 0:
                results = run(args, file_path, planner, scenario_indices, iteration=0, codes=[baseline_code] * len(scenario_indices), commands=None, seed=0)
            ...
            results_1 = run(args, file_path, planner, scenario_indices_1, iteration=0, codes=[baseline_code] * len(scenario_indices_1), commands=None, seed=0)
            ...
            results_2 = run(args, file_path, planner, scenario_indices_2, iteration=0, codes=[baseline_code] * len(scenario_indices_2), commands=None, seed=0)
            ...
            for iteration in range(1, int(args.rs_iterations) + 1):
                if use_baseline:
                    codes_1, commands_1 = [baseline_code] * len(scenario_indices_1), None
                else:
                    codes_1, commands_1 = future_1.result()
                results_1 = run(args, file_path, planner, scenario_indices_1, iteration=iteration, codes=codes_1, commands=commands_1, seed=iteration)
                log_paths_1 = save_results(results_1)
                ...
                if use_baseline:
                    codes_2, commands_2 = [baseline_code] * len(scenario_indices_2), None
                else:
                    codes_2, commands_2 = future_2.result()
                results_2 = run(args, file_path, planner, scenario_indices_2, iteration=iteration, codes=codes_2, commands=commands_2, seed=iteration)
                log_paths_2 = save_results(results_2)
                if iteration < int(args.rs_iterations):
                    scenario_indices_2 = [scenario_indices_2[i] for i in range(len(results_2)) if not results_2[i]['task_result']]
                    if not use_baseline:
