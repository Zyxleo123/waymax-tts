...
class VecSceneGemmaVLA(nn.Module):
    def __init__(
        ...
        predict_subgoal: bool = False,
    ):
        super().__init__()
        self.num_scene_tokens = num_scene_tokens
        self.predict_subgoal = predict_subgoal
        ...
        self.scene_tokenizer = SceneTokenizer(
            ...
            cond_dim=token_dim,
            num_tokens=num_scene_tokens + 1 if self.predict_subgoal else num_scene_tokens,
        )
        ...
        if self.predict_subgoal:
            print("VLA initialized with subgoal prediction")
            self.subgoal_prediction_head = MLP([token_dim, hidden_dim, hidden_dim, 2], end_layer_activation=False)
        else:
            print("VLA initialized without subgoal prediction")
    ...
    def _build_init_kwargs_from_config(config: Mapping[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "predict_subgoal": bool(pick("predict_subgoal", False)),
        }
    ...
    def forward(
        ...
        if self.predict_subgoal:
            subgoal_pred = 100.0 * self.subgoal_prediction_head(subgoal_token)
            subgoal_xy = 100.0 * input_features["subgoal_xy"]
            # subgoal_loss = nn.functional.mse_loss(subgoal_pred, subgoal_xy, reduction="mean")
            subgoal_loss = nn.functional.huber_loss(subgoal_pred, subgoal_xy, reduction="mean")
        ...
        return {
            ...
            "subgoal_loss": subgoal_loss if self.predict_subgoal else torch.tensor(0.0, device=loss.device),
        }
    ...
    def generate_predictions(
        ...
        return {
            ...
            "subgoal_pred": subgoal_pred.detach().float().cpu().numpy() if self.predict_subgoal else None,
        }
    ...
    def state_dict(
        ...
        if only_trainable:
            keep_prefixes = [
                "scene_tokenizer.",
                "scene_token_self_attention.",
                "subgoal_prediction_head.",
            ]
            filtered = {
                k: v.cpu()
                for k, v in state.items()
                if (
                    any(k.startswith(p) for p in keep_prefixes)
                    or "lora_" in k
                )
            }
            return filtered
