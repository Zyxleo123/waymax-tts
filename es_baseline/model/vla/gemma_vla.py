...
class VecSceneGemmaVLA(nn.Module):
    def __init__(
        ...
        # special_tokens = {"additional_special_tokens": ["<INSTRUCTION>", "<SUBGOAL>"]}
        # num_added = self.tokenizer.add_special_tokens(special_tokens)
        # if num_added > 0:
        #     self.llm.resize_token_embeddings(len(self.tokenizer))
        # self.inst_token_id = self.tokenizer.convert_tokens_to_ids("<INSTRUCTION>")
        # self.subgoal_token_id = self.tokenizer.convert_tokens_to_ids("<SUBGOAL>")
    ...
    def generate_inst_subgoal_predictions(
        self,
        input_features: Mapping[str, torch.Tensor],
        prompt_ids: torch.Tensor,  # [B, P]
        prompt_mask: torch.Tensor,  # [B, P]
        ...
        for text in decoded:
            instruction_text, subgoal_text = self.split_instruction_subgoal_text(text)
            instruction_texts.append(instruction_text)
            subgoal_texts.append(subgoal_text)
        return instruction_texts, subgoal_texts

    def split_instruction_subgoal_text(self, text: str) -> tuple[str, str]:
